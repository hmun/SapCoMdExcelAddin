...
Public Class SAPCostCenter
    ...
    Public Function createMultiple(pData As TSAP_CCData, Optional pOKMsg As String = "OK") As String
        createMultiple = ""
        Try
            oRfcFunction = destination.Repository.CreateFunction("BAPI_COSTCENTER_CREATEMULTIPLE")
            RfcSessionManager.BeginContext(destination)
            Dim oRETURN As IRfcTable = oRfcFunction.GetTable("RETURN")
            Dim oCOSTCENTERLIST As IRfcTable = oRfcFunction.GetTable("COSTCENTERLIST")
            oRETURN.Clear()
            oCOSTCENTERLIST.Clear()
            ...
            For Each aKvP In pData.aData.aTDataDic
                Dim oCOSTCENTERLISTAppended As Boolean = False
                aTDataRec = aKvP.Value
                For Each aTStrRec In aTDataRec.aTDataRecCol
                    Select Case aTStrRec.Strucname
                        Case "COSTCENTERLIST"
                            If Not oCOSTCENTERLISTAppended Then
                                oCOSTCENTERLIST.Append()
                                oCOSTCENTERLISTAppended = True
                            End If
                            oCOSTCENTERLIST.SetValue(aTStrRec.Fieldname, aTStrRec.formated())
                    End Select
                Next
            Next
            ' call the BAPI
        ...
        Catch Ex As System.Exception
            MsgBox("Error: Exception " & Ex.Message, MsgBoxStyle.OkOnly Or MsgBoxStyle.Critical, "SAPCostCenter")
    ...
    Public Function changeMultiple(pData As TSAP_CCData, Optional pOKMsg As String = "OK") As String
        changeMultiple = ""
        Try
            oRfcFunction = destination.Repository.CreateFunction("BAPI_COSTCENTER_CHANGEMULTIPLE")
            Dim oRETURN As IRfcTable = oRfcFunction.GetTable("RETURN")
            Dim oCOSTCENTERLIST As IRfcTable = oRfcFunction.GetTable("COSTCENTERLIST")
            oRETURN.Clear()
            oCOSTCENTERLIST.Clear()
